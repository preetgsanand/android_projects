...
import vince.jobtracking.Database.Report;
...
public class Utils {
    ...
    public static List<Report> JSONArrayToReport(JSONArray response) {
        List<Report> reports = new ArrayList<>();
        try {
            for(int i = 0 ; i < response.length() ; i++) {
                JSONObject reportObj = response.getJSONObject(i);
                Report report = new Report();
                report.setWebid(reportObj.getLong("id"));
                report.setContent(reportObj.getString("content"));
                report.setAdded(Utils.DateToLong(reportObj.getString("added")));
                report.setReciever(reportObj.getInt("sender"));
                if(!(reportObj.get("reciever").equals(null))){
                    report.setReciever(reportObj.getInt("reciever"));
                }
                report.setReportType(reportObj.getInt("reportType"));
                report.setSubject(reportObj.getString("subject"));
            }
            return reports;
        }
        catch (Exception e) {
