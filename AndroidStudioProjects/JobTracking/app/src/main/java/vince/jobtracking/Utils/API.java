...
public class API {
    ...
    public void reportlist() {
        ...
        new Thread() {
            public void run() {
                JsonArrayRequest jsonArrayRequest = new JsonArrayRequest(Request.Method.GET,
                        context.getResources().getString(R.string.report_list),
                        null,
                        new Response.Listener<JSONArray>() {
                            @Override
                            public void onResponse(final JSONArray response) {
    ...
    private void parseReportJsonArray(JSONArray response) {
        if(response.length() == 0) {
            postResult(0);
        }
        else {
            try {
                List<Report> reports = Utils.JSONArrayToReport(response);
                if(reports != null) {
                    Report.saveInTx(reports);
                    postResult(1);
                }
            }
            catch (Exception e) {
                postResult(-1);
                Log.e("Report Parse Json",e.toString());
            }
        }
    }
